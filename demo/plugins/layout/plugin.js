...
tinymce.PluginManager.add('layout', function(editor, url) {
    ...
    var layout_opt = editor.getParam('layout_options', {clearStyle:[],filterTags:['table','tbody','td','tr'],style:{'text-align':'justify','text-indent':'2em','line-height': 1.5},tagsStyle:{'table':{"line-height": 2}}});
    var layout_filterTags={};
    for( let key in layout_opt.filterTags){layout_filterTags[layout_opt.filterTags[key].toUpperCase()]=true;}
    var doAct = function () {
        var dom = editor.dom;
        ...
        function setTagsStyle(_block){
            for(let key in layout_opt.tagsStyle){
                let ckeyList = key.split(',');
                 for(let ckey in ckeyList){
                    _block.tagName==ckeyList[ckey].toUpperCase()? setStyleFun(_block,layout_opt.tagsStyle[key],true):'';
                 }
            }
        ...
        function setStyleFun(_block,_style,isTags){
            ...
            dom.setStyle(_block, 'text-indent', layout_opt.style['text-indent']?layout_opt.style['text-indent']: kv?kv:'2em');
          }
        }
        var act = '';
        global$1.each(blocks, function (block) {
                if(act==''){if(dom.hasClass(block,'layoutFV')){act =  'remove'; dom.removeClass(block,'layoutFV')}else{ act =  'add'; dom.addClass(block,'layoutFV')}}
                if( act=='add'){
                    !layout_filterTags[block.tagName]?setStyleFun(block,layout_opt.style):'';
                    layout_opt.clearStyle?clearStyleFun(block):'';
                    layout_opt.tagsStyle?setTagsStyle(block):'';
                }else{
                    !layout_filterTags[block.tagName]?removeStyleFun(block,layout_opt.style):'';
                }
        });
    };
